...
sem filler). Skill em `.Codex/skills/caveman/SKILL.md` (MIT, © 2026 Julius Brussee).
...
Se `.Codex/.caveman-setup-done` não existir, pergunte ao desenvolvedor se ele
deseja instalar o **caveman completo** (plugin + hooks globais: auto-ativação e
statusline) via:
  curl -fsSL https://raw.githubusercontent.com/JuliusBrussee/caveman/main/install.sh | bash
Registre a decisão em `.Codex/.caveman-setup-done` (não versionado) para não
